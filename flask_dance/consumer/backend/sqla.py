...
class SQLAlchemyBackend(BaseBackend):
    ...
    def get(self, blueprint, user=None, user_id=None):
        ...
        the "token()" property (see base.py) is used to check your
        Flask server's cache and database for any records associated with the
        current_user. The `user` and `user_id` parameters are actually not
        set in that case (see base.py:token(), that's what calls this
